...
namespace Padel.Grpc.Core
{
    public static class ServerCallContextExtension
    {
        ...
        {
            var userIdStr = context.RequestHeaders.FirstOrDefault(entry => entry.Key == Constants.ErrorHeaderKey)?.Value;
            if (string.IsNullOrWhiteSpace(userIdStr))
            {
                var metadata = new Metadata {{Constants.ErrorHeaderKey, $"missing header '{Constants.ErrorHeaderKey}'"}};
